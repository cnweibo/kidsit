...
<body ng-app="kidsitApp" ng-controller="playListsCtrl as wplvm">
	<div id="isLoading" ng-show="isLoading"><img src={{asset('htmlapp/assets/images/ajax-loading-98.gif')}} alt=""></div>	
@overwrite
@section('content')
...
<script type="text/ng-template" id="singleword.html">
	...
	<div class="container" mp3-player >
		...
		<div>
		<h1 style="display:inline;margin-right:20px"><a href="{{route('yinbiao.show',$yinbiao->id,['class'=>'yinbiaoatag'])}}">{{$yinbiao->name}}</a> </h1>
		<span id="yinbiao_1"><i class="clickable glyphicon glyphicon-volume-up"></i>点击发音</span>
		<span>所属类别： <a href="{{route('yinbiaocategory.show',$yinbiao->yinbiaocategory->id,['class'=>'yinbiaocatatag'])}}">{{$yinbiao->yinbiaocategory->ybcategory}}</a>  </span>
						...
						<div class="panel-heading">
							<div class="row">
								<div class="wordblock col-xs-4 col-sm-4 col-md-4 col-lg-4">
									<h3 class="panel-title"><a href="{{route('fayinguize.show',$fayinguize->id,['class'=>'yinbiaocatatag'])}}">{{$fayinguize->title}}</a></h3>
								</div>
...
@section('scripts')
<!-- inject:js -->
<script src="/build/js/yinbiaoApp.min.js"></script>
<!-- endinject -->
<!-- inject:css -->
<!-- endinject -->

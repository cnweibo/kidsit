		...
		{{--*/$layoutloopcount++;/*--}}
				<div class="col-xs-6 col-sm-6 col-md-6 col-lg-6">
					<div class="panel panel-success">
						...
						<h3 class="panel-title"><a href="{{route('yinbiaocategory.show',$ybcategory->id)}}">{{$ybcategory->ybcategory}}</a></h3>
						</div>
						<div class="panel-body">
							<h4>{{$ybcategory->description}}</h4>
							@foreach ($ybcategory->yinbiao as $yinbiao)
								<span style="font-family:Lucida Sans Unicode,Arial Unicode MS;letter-spacing: 5px;font-size:20px"> <a href="{{route('yinbiao.show',$yinbiao->id)}}">{{$yinbiao->name}}</a> </span>			
							@endforeach
						</div>
				    </div>	
				</div>	
			...
			@endif	
			<p><a href="{{route('yinbiaocategory.index')}}">浏览全部英标类别</a></p>

...
use Kidsit\Models\Math\Mathskillcat;
use Kidsit\Models\Math\Mathskill;

class AdminMathskillsController extends Controller
{
    ...
    public function show($id)
    {
        if (is_numeric($id)){
            ...
            try {
                list($catlable, $catsubid) = preg_split('/\./', $id);
            }
            catch(Exception $e){
                return Response::json(['resp' => ['code' => 400, 'message' => '未找到id为' . $id .'的资源']], 404);
            }
            $mathskillcat_id = Mathskillcat::where('catlabel','=',$catlable)->first()->id;
            $mathskillcat = Mathskill::where('catsubid' , '=', $catsubid)->where('mathskillcat_id','=',$mathskillcat_id)

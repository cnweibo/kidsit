...
(function () {
    ...
    function studentListCtrl($scope,$window,toastr,$location,khttp,$q) {
        ...
        var vm = this;
        var promise;
        vm.currentPromise = promise = khttp.getAll(basetempurl+"/admin/api/system/student/");
        promise.then(
		...
		vm.checkAndSaveStudent = function(data,field,student) {
			var d = $q.defer();
			student[field] = data;
			vm.currentPromise = promise = khttp.update(basetempurl+"/admin/api/system/student/"+student.id,student);
			promise.then(
		...
		vm.deleteStudent = function(student){
			...
			khttp.destroy(basetempurl+"/admin/api/system/student/",student).then(
		...
		vm.loadStudentSelectableBelongingClasses = function () {
			khttp.getAll(basetempurl+"/admin/api/system/classroom").then(

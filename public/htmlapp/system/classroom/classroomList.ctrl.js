...
(function () {
    ...
    function classroomListCtrl($scope,$window,toastr,$location,khttp,$q,simplevalidate) {
        ...
        var vm = this;
        var promise;
        vm.currentPromise = promise = khttp.getAll(basetempurl+"/admin/api/system/classroom/");
        promise.then(
        ...
        vm.deleteClassroom = function(classroom){
            ...
            khttp.destroy(basetempurl+"/admin/api/system/classroom/",classroom).then(
        ...
        var returned = simplevalidate.dovalidate(rules,data,
                                                basetempurl+'/admin/api/system/classroom/');
        returned.then(
            function (response) {
                if (response){
                    // already exist
                    ...
                    vm.currentPromise = promise = khttp.update(basetempurl+"/admin/api/system/classroom/"+classroom.id,classroom);
                    promise.then(
        ...
        vm.loadClassroomSelectableOwners = function () {
            khttp.getAll(basetempurl+"/admin/api/system/teacher").then(

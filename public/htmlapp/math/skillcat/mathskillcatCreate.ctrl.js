...
(function () {
   ...
   //      ];
        ...
        vm.createMathskillcat = function (form) {
			...
			vm.currentPromise = promise = khttp.store(basetempurl+"/admin/api/math/skillcat",vm.newMathskillcat);
			promise.then(
        ...
        $scope.canUseThisName = function (thisname) {
			var d = $q.defer();
			var promise;
			vm.verfiycatlabelpromise = promise = khttp.getOne(basetempurl+'/admin/api/math/skillcat/',thisname);
			promise.then(
				function (a) {
					return d.reject();
				},

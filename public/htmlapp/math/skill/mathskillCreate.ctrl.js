...
(function () {
    ...
    function mathskillCreateCtrl($scope,khttp,$window,toastr,$location,$q,$timeout) {
        ...
        vm.currentPromise = promise = khttp.getAll(basetempurl+"/admin/api/system/grade/");
        promise.then(
		...
		khttp.getAll(basetempurl+'/admin/api/math/skillcat/')
			.then(
				function (response) {
        ...
        vm.createMathskill = function (form) {
			...
			vm.currentPromise = promise = khttp.store(basetempurl+"/admin/api/math/skill",vm.newMathskill);
			promise.then(
        ...
        $scope.canUseThisName = function (thisname) {
			...
			vm.verfiycatsubidpromise = promise = khttp.getOne(basetempurl+'/admin/api/math/skill/',vm.newMathskill.skillcatlabel+'.'+thisname);
			promise.then(
				function (a) {
					return d.reject();
				},
